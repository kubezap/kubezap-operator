...
**Planning-only edits stay staged, not PR'd, until you're told otherwise.** `/groom-backlog`, `/plan-pi`, `/plan-parallel`, and any other planning-only pass edit `planning/` files — leave those edits staged (`git add`, not committed) rather than committing, pushing, or opening a PR on your own initiative. Wait for one of two triggers: the user explicitly asks for a PR of the planning docs alone, or the session moves on to `/dispatch-work`. `/dispatch-work` gives the actual implementation work its own PR per story — never a direct merge to `main` (see `/dispatch-work`'s process). Once the user confirms all of that batch's story PRs are merged, commit the accumulated staged planning edits (updated to reflect final `Done`/merged status) as one final PR.

**Batch planning-only PRs.** When a planning-only PR does go out (per the trigger above), land everything accumulated since the last one as a single PR, not one per action — separate PRs here mostly just create avoidable merge-conflict churn on shared files like `planning/backlog/backlog.md`. Only split into separate PRs when there's a real reason to (e.g. one needs to merge before a dependent implementation PR can, or the user asks to review a piece in isolation).
...
- Go 1.27 (see `go.mod` for the exact pinned patch version — kept current with upstream security releases; see `SECURITY.md`'s Dependency Vulnerability Management section)
