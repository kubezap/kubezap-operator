...
.PHONY: vulncheck
vulncheck: govulncheck ## Scan for known vulnerabilities reachable from this module's code
	$(GOVULNCHECK) ./...
...
.PHONY: build-cli
build-cli: ## Build the kubezap CLI binary (bin/kubezap).
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kubezap-builder
	$(CONTAINER_TOOL) buildx use kubezap-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kubezap-builder
	rm Dockerfile.cross
