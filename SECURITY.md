...
**Note on CodeQL / GitHub Advanced Security**: this repository does not run CodeQL SAST scanning. CodeQL's results (and Trivy's SARIF upload, which fed the same Code Scanning alerts view) require GitHub Advanced Security, which isn't available on a private repository without a paid license. If this repository moves to a plan with Advanced Security (or goes public, where GHAS is free), re-adding `.github/workflows/codeql.yml` and restoring Trivy's `format: sarif` + SARIF upload step is a small, mechanical change — see git history around 2026-09-19 for the removed configuration.
