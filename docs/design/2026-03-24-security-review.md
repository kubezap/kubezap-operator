...
All questions resolved — rationale for each below.
...
All findings were fixed (2026-03-27); the SSRF two-layer mitigation (blocklist + separate `http-executor` binary with minimal RBAC) is the architecture in production use today, and has since been hardened further — see `docs/design/2026-09-11-executor-egress-networkpolicy.md` (network-layer defense-in-depth for the same SSRF surface) and `docs/design/2026-09-11-webhook-gateway-trust-boundary.md`.
