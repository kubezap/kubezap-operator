...
| 2026-09-18 | [Self-Managed Webhook Admission Certs](2026-09-18-self-managed-webhook-certs.md) | Approved | In-process self-signed cert generation/rotation + `caBundle` patching for the operator's own `ValidatingWebhookConfiguration`, replacing `hack/gen-webhook-certs.sh` and its duplicated dev/e2e/benchmark cert-provisioning logic. Rejects wiring up cert-manager (turns an optional convenience into a hard install-time dependency for every install path). |
