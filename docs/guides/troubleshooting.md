...
| `TLS handshake failed` / `certificate signed by unknown authority` | The target uses a self-signed or private CA. HTTP steps have no CA-bundle override today, and TLS verification is always enforced for the standard Flow-driven path — `--allow-tls-skip-verify` on the `http-executor` is dev-only and not reachable via any Trigger/Flow field. There's no fix today short of putting the target behind a publicly-trusted CA. For a broker Integration (Kafka/AMQP/NATS) hitting this instead, set `Integration.spec.{kafka,amqp,nats}.tls.caSecretRef` to the Secret containing your private CA. |
