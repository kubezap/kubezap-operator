...
> **Note:** By default, the gateway never trusts `X-Forwarded-For`/`X-Real-IP` — it always uses the TCP peer address, which is the Ingress controller's/load balancer's own IP if you're behind one, not the original client's. This is a deliberate, safe default: if the gateway trusted these headers unconditionally, any caller reaching it directly could set either one to an allowed IP and bypass the allowlist entirely. If you're behind a reverse proxy that forwards the real client IP via `X-Forwarded-For` (or `X-Real-IP`), pass `--trusted-proxy-cidrs` (a comma-separated list of CIDRs) to the webhook gateway with that proxy's pod/service CIDR; the header is then honored only when the immediate connection actually comes from a trusted CIDR. This is gateway-wide, not per-Trigger.
>
> **Multi-hop proxy chains**: `--trusted-proxy-cidrs` also supports chains of more than one intermediate hop (for example CDN → WAF → ingress → gateway), as long as every intermediate hop's address falls within `--trusted-proxy-cidrs`. The gateway walks `X-Forwarded-For` from the right, popping each entry as long as it is itself within `trustedProxies` (that entry is a trusted hop's own address, appended by the hop before it), and stops at — and returns — the first entry that is *not* within `trustedProxies`. That is treated as the real client's address, since it's the address the nearest trusted hop actually observed. For a single-hop deployment (the common case) this is unchanged: the right-most entry is the real client's address as the trusted hop observed it, is not itself in `trustedProxies`, and is returned immediately. The walk is capped at a fixed, generous, non-configurable maximum number of entries as defense-in-depth against a pathological header — this is a safety bound, not a trust-model setting.
>
> **Scope `--trusted-proxy-cidrs` tightly.** Every CIDR you add is a hop whose *self-reported* address the gateway will trust and pop past to keep walking left — so the more hops (and the broader the ranges) you trust, the more of the chain an entity sitting anywhere in that trusted path can influence just by relaying a header value unchanged. Include only the actual, known pod/service CIDRs of your real intermediate proxies (CDN, WAF, ingress), never broad internal ranges "to be safe" — a broad range widens both which peers can inject `X-Forwarded-For` at all *and* how many hops of that header the walk will trust before stopping, compounding the risk more than it did with the old single-hop behavior.
