...
Never open this for an RC. This step is also independent of the operator's own release-readiness gate (`EPIC-001`/`STORY-007`, closed 2026-09-26 — KubeZap's `community-operators` PR for `v0.2.0` merged and it's now listed on `https://operatorhub.io/operator/kubezap`); don't open an OperatorHub PR for a future release without confirming that gate (or its equivalent for whatever's changed) is still clean.
