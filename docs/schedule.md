...
- [ ] **HIGH** `internal/gateway/kafka/watcher.go`: Uses a polling loop (`time.NewTicker(30 * time.Second)`) rather than a controller-runtime informer or watch. New/deleted/modified Triggers are picked up with up to 30 s delay. Should use an informer-based watch (same pattern as `webhook/watcher.go`). Scope: Kafka only; apply same fix to NATS/AMQP watchers when those are implemented.
...
- [ ] **HIGH** `api/v1alpha1/trigger_types.go` + `webhook/handler.go`: Implement `basic` auth (Option 2: separate `usernameKey`/`passwordKey` in secret). Remove `mtls` from `WebhookAuth.Type` enum — mTLS does not apply to HTTP webhooks. Add TLS termination support to webhook gateway server (server cert/key for end-to-end TLS / OpenShift re-encrypt routes).
