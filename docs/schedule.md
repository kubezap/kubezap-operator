...
- [ ] **FOLLOW-UP DECISION** — `FlowStep.params` (`[]ParamValue`, distinct from `FlowRunSpec.params`): purpose is unclear from docs or code (one-line description, no example uses it, not addressed by the design record above, explicitly out of scope for the implementation above). Decide: implement with a clarified purpose, or remove from the API (`api/v1alpha1/flow_types.go` `FlowStep.Params` field, `docs/api/flow.md` reference).
- [ ] **PHANTOM FIELD — found during §31 implementation, not fixed** — `HTTPAction.BodyFrom` is documented/defined in the CRD schema but is never read anywhere in the controller (confirmed via grep: zero matches for `.BodyFrom` outside its type definition). Decide: implement or remove.
...
- [ ] **BUG — found while fixing the item above, NOT YET FIXED, bigger than originally filed** — the **plugin-type** publish path (`integration.Spec.Plugin != nil` branch of `executePublishStep` / `doPluginPublish`) doesn't send `Topic` at all, interpolated or not. `docs/api/integration.md`'s documented Publisher contract requires the controller to `POST` a JSON envelope to `/publish`: `{"integration": ..., "namespace": ..., "destination": "<topic>", "headers": {...}, "body": "..."}`. The actual implementation POSTs the raw `body` string as the HTTP request body with `headers` as HTTP headers — no JSON envelope, no `integration`/`namespace`/`destination` fields at all. Any plugin built to the documented contract cannot know which topic/queue/subject to publish to; the field is fully disconnected end-to-end, not just missing interpolation. Zero existing test coverage of `doPluginPublish` (confirmed by grep). This is a wire-protocol mismatch between controller and plugin, not a one-line fix — it needs its own design record (`docs/guides/design-process.md`) before implementation, since it changes an external dependency's contract per that guide's trigger list. Decide: fix the controller to emit the documented envelope, or correct the docs to describe what's actually sent (and then decide separately whether `Topic` interpolation matters for the plugin path too).
