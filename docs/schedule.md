...
- [ ] **mTLS for webhook gateway** — server-side TLS termination (cert + key via user-provided Secret, cert-manager compatible) and optional client certificate verification (inbound mTLS). Documented in `docs/overview.md#tls-and-mtls` via `kubezap.io/webhook-mtls-ca-secret` annotation; implementation pending. Note: this is distinct from the `WebhookAuth.type` field — TLS termination is a transport-layer concern, not an application-auth type. See `docs/tech-debt/pending-input-required.md#c-mtls-auth--tls-termination` for prior design decision.
...
- [ ] `internal/gateway/amqp/watcher.go` and `internal/gateway/nats/watcher.go`: Both use a polling loop (`time.NewTicker(30 * time.Second)`) rather than a controller-runtime informer or watch — same issue that was fixed in the Kafka gateway. Reaction time to Trigger changes is up to 30 s. Should be refactored to informer/cache pattern (mirrors `internal/gateway/kafka/watcher.go` post-refactor).
...
- [ ] **HIGH** `internal/gateway/amqp/watcher.go` and `internal/gateway/nats/watcher.go` `startSubscription`: both use `context.WithCancel(context.Background())` instead of `context.WithCancel(ctx)` — subscriptions are not cancelled on pod shutdown, breaking graceful drain. Same bug that was fixed in `kafka/watcher.go`. One-line fix per file. See `docs/tech-debt/gateway-shutdown-correctness.md#issue-1`.
- [ ] **MEDIUM** `internal/gateway/nats/watcher.go` `startSubscription`: NATS NKey/JWT credentials written to `/tmp/nats-creds-<trigger.Name>.creds` — predictable path, no cleanup on subscription stop, name collision risk across namespaces. Replace with `os.CreateTemp` + cleanup, or use in-memory credential loading. See `docs/tech-debt/gateway-shutdown-correctness.md#issue-3`.
- [ ] **HIGH** `internal/controller/integration_controller.go` `reconcileKafkaGateway`: Kafka gateway SA, Role, and RoleBinding are created without `ctrl.SetControllerReference` on the Integration — they are orphaned on Integration delete and accumulate indefinitely. Fix requires adding `delete` verb to controller RBAC first (line 262). See `docs/tech-debt/rbac-ownership-gaps.md#issue-1`.
- [ ] **LOW** `test/e2e/e2e_test.go`: E2E suite validates controller pod readiness and metrics endpoint only; TODO placeholders at lines 258–265 indicate actual Trigger/Flow/FlowRun workflow scenarios are not implemented. Add at minimum: webhook trigger → transform → http step → verify FlowRun Succeeded.
...
- [ ] `docs/api/trigger.md`: `WebhookAuth` spec reference lacks the full list of supported auth types and field descriptions. Types are documented in `docs/guides/webhook-security.md` but the core API reference is incomplete for users who don't follow cross-links.
- [ ] `docs/api/flow.md`: `type: publish` step action missing implementation detail — how the controller calls the Integration's `/publish` endpoint, error handling, interaction with retry policy.
- [ ] `docs/api/integration.md`: KEDA ScaledObject for Kafka gateway not documented anywhere. Architecture doc mentions it but no guide or API doc explains the ScaledObject config or `kubezap_kafka_consumer_lag` metric.
- [ ] `docs/guides/observability.md`: ServiceMonitor "not auto-created" caveat is buried in the guide; users who skim to the Prometheus section won't see it until after deployment. Promote to a callout box near the top of the Prometheus section.
