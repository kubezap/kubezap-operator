...
- [ ] **PHANTOM FIELD — found during §31 implementation, not fixed** — `PublishAction.Topic` is documented as supporting `$(...)` interpolation, but `executePublishStep`/`r.publishToKafka(...)` pass `step.Action.Publish.Topic` through raw, never via `substituteVars`. A topic like `orders.$(trigger.body.region)` would silently publish to a literal, unresolved topic name. Fix: thread `substituteVars` over `Topic` before use, or correct the docs if interpolation there was never intended.
