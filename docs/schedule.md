...
- [ ] **IMPLEMENTATION** — Build the design in `docs/design/2026-09-10-flow-parameters.md`. Task breakdown:
  1. `internal/controller/flowrun_controller.go`: add `resolveFlowParams` (or similar) implementing the 5-step resolution order from the design record; call it once at the `""|"Pending" → "Running"` transition, before step dispatch.
  2. Fail fast via the existing `failFlowRun` path when a `required` param resolves to nothing; message must name the missing parameter.
  3. Thread the resolved `map[string]string` into `substituteVars` as a new `$(params.<name>)` case (mirrors the existing `steps.*.results.*` pattern).
  4. Thread the same map into `evaluateWhen`'s CEL activation as a new `params` variable (`map<string, string>`, alongside `trigger`/`steps`).
  5. Ginkgo coverage per the design record's implementation notes: explicit-override-wins, JSON and form-urlencoded auto-derivation, default fallback, required-and-missing fails before any step runs, `$(params.x)` and CEL `params.x` agree.
  6. Live-verify against the real cluster: re-run at least one of the ~7 affected `flow.md` examples end-to-end (don't just assume they now work because they use the same-name convention).
  7. Docs: remove the "not implemented" warnings added in §31 from `docs/api/flow.md` (top-of-page banner, `ParamDeclaration`/`ParamValue` sections, Quick Reference note) and `docs/api/flowrun.md` (`params` field row); document the actual resolution order (explicit > auto-derived > default > fail/empty) in both.
  8. Re-check the 7 worked examples in `flow.md` against the implemented behavior; fix any that don't follow the same-name auto-derivation convention.

- [ ] **FOLLOW-UP DECISION** — `FlowStep.params` (`[]ParamValue`, distinct from `FlowRunSpec.params`): purpose is unclear from docs or code (one-line description, no example uses it, not addressed by the design record above). Decide: implement with a clarified purpose, or remove from the API (`api/v1alpha1/flow_types.go` `FlowStep.Params` field, `docs/api/flow.md` reference).
