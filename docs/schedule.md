...
- [ ] **SECURITY/ARCH** — Extract HTTP step execution into a separate `kubezap/http-executor` binary. Controller resolves secrets in-memory, sends fully-resolved HTTP request to executor via internal `POST /execute` RPC (HTTP/JSON, not gRPC). Credentials never written to etcd. Executor has no secrets RBAC, no RBAC management — only makes outbound HTTP calls with SSRF blocklist. New binary: `cmd/http-executor/main.go`, package `internal/executor/http/`. One Deployment per namespace. Channel security: NetworkPolicy (mandatory) restricts executor ingress to controller pod only; mTLS (opt-in via `--executor-mtls=true`) for clusters without a service mesh. See §C1 rationale.
