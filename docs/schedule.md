...
- [ ] **TECH DEBT** — Related and lower-severity, NOT addressed by the fix above: the `type: publish` Kafka producer cache (`internal/controller/flowrun_controller.go`'s `kafkaProducers`, `kafkaProducerIdleTTL` 10 minutes — a different code path from the gateway watchers fixed above, used when a Flow step publishes *to* Kafka) means even a secret change that *is* now detected by the gateway's fresh Integration/Trigger reconcile won't affect an already-open cached producer until it idles out. Worth deciding whether producer recreation should be forced on Integration secret-related field changes, separately from idle eviction.
