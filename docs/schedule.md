...
- [ ] **TECH DEBT — found while removing `FlowStep.params` 2026-09-11** — `charts/kubezap-operator/crds/` (renamed from `charts/kubezap/crds/` 2026-09-11, see the chart-rename item below) and `bundle/manifests/` (the Helm chart and OLM bundle copies of the CRD schemas) are significantly out of sync with `config/crd/bases/`, the source of truth regenerated by `make manifests`. Neither copy is auto-synced by any Makefile target — every prior CRD schema change appears to have been applied to `config/crd/bases/` only. Confirmed drift (non-exhaustive, found via direct diff against `automation.kubezap.io_flows.yaml`/`_flowruns.yaml` only — other CRDs not yet checked): `HTTPAction.integrationRef` missing entirely, `TriggerData`'s resource-trigger fields (`eventType`, `resourceName`, `resourceNamespace`, `resourceAPIVersion`, `resourceKind`) missing, the trigger-type enum still lists a stale `pubsub` value instead of the real `kafka`/`amqp`/`nats`/`resource` set, `FlowRunStatus.observedGeneration` missing, and several field descriptions are stale/shortened versions of the current ones. Anyone installing via Helm or OLM today would get CRDs missing fields the controller (and these docs) assume exist. Fix: add a Makefile target that copies/regenerates `config/crd/bases/*` into both destinations as part of `make manifests` (or `make bundle`/a new `make manifests-all`), then do a one-time full resync; re-check all 4 CRDs (`Trigger`, `Flow`, `FlowRun`, `Integration`), not just the two touched by this session's changes.
