...
- [x] **BUG/DOC FIX (fixed 2026-09-11)** — `docs/api/flow.md`'s own "Using Trigger Data in CEL Conditions" section documented `trigger.body.eventType == "order.placed"` as a working CEL `when:` example — this does not work for **any** content type, not just XML: `evaluateWhen`'s CEL activation map always sets `triggerMap["body"]` to the raw string `triggerData.Body` (`internal/controller/flowrun_controller.go` ~line 1749), never a parsed structure, regardless of `Content-Type`. Dot-path field access on it raises a CEL runtime error, which the controller treats as a `when expression error` and fails the step (or the whole FlowRun, depending on `OnFailure`/`FailurePolicy`). This is a real, previously-undiscovered functional gap: `$(...)` interpolation supports full dot-path into a JSON/form-urlencoded body, but CEL conditions never do, for any format. Fixed the doc to state this plainly, with the one real workaround (declare a same-named top-level `param`, auto-derived from the body per the §31 Flow-parameters work, then test `params.<name>` — flat top-level fields only, nested fields aren't reachable from CEL today) and a corrected example using `.contains()` on the raw string. Added a matching bullet to the `## Limitations` section. Found by manually reading the controller code, not by the subagent's sweep.
