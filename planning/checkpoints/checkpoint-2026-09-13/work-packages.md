...
All three fit within the 3-parallel-worker cap (Rate Limit Guardrails / this skill's own guardrail). Each was built in its own isolated worktree, verified independently (`go build ./...`, `make generate && make manifests` showed no drift, `make test`, `make lint` all clean against a local integration of all three), then pushed and opened as three separate PRs per this repo's normal convention (never merged directly to `main`).

**Status note:** this file plus the `backlog.md`/story-file status edits for STORY-012/019/020 are staged locally, not committed — per `CLAUDE.md`'s planning-PR guidance, they'll land as one final planning-only PR once PR #202/#203/#204 are all confirmed merged.
